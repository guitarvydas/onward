...
%%%% cull style
\begin{wrapfigure}[1]{r}{0.6\textwidth}
   \centering
...
\begin{wrapfigure}[3]{r}{0.6\textwidth}
    \centering
...
component is instantiated four (4) times. 
...
1, the "Echo" code creates four (4) instances, each being unique, but,
...
\underline{LLM0D}\cite{llm0d} a single component diagram that uses the Go programming language as a host language to connect to openai's LLM for experimention,
...
\underline{dc0d}\cite{dc0d} game based on Chris Marten's "dungeon crawler" example\cite{ceptre_paper}; 
\underline{gen0D} which generates leaf components that are used in conjunction with dc0d,
\underline{0D odin}\cite{odin0d} a DPL engine written in the Odin programming language,
